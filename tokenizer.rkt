...
(provide (contract-out [make-tokenizer
...
(module+ test
  ...
  (check-equal?
   ...
   (list (token 'SEXP-TOK " (+ 6 7) " #:position 3 #:line 1 #:column 2 #:span 9 #f)))
  (check-equal?
   (apply-tokenizer-maker make-tokenizer "hi")
   (list (token 'CHAR-TOK "h" #:position 1 #:line 1 #:column 0 #:span 1 #f)
         (token 'CHAR-TOK "i" #:position 2 #:line 1 #:column 1 #:span 1 #f))))
